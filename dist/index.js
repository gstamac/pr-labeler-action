...
function action(context = github.context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const config = yield config_1.default(octokit, configPath, context.repo, head_ref, defaultConfig);
...
function getLabelsToAdd(config, headBranchName, baseBranchName) {
    const labelsToAdd = [];
    for (const label in config) {
        const isObject = config[label] !== null && typeof config[label] === 'object' && !Array.isArray(config[label]);
        const isBase = isObject && 'base' in config[label];
        const patterns = arrayify_1.arrayify(isObject ? Object.values(config[label])[0] : config[label]);
        const matches = matcher_1.default([isBase ? baseBranchName : headBranchName], patterns);
        if (matches.length > 0) {
